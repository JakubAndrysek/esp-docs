...
function byteArrayToBase64(bufArr) {
  if (typeof Buffer !== "undefined") {
    ...
    for (let i = 0; i < buf.length - 2; i += 3) {
      ...
      result += b64dict[(buf[i] & 3) << 4 | buf[i + 1] >> 4];
      result += b64dict[(buf[i + 1] & 15) << 2 | buf[i + 2] >> 6];
      result += b64dict[buf[i + 2] & 63];
    }
    if (buf.length % 3 === 1) {
      result += b64dict[buf[buf.length - 1] >> 2];
      result += b64dict[(buf[buf.length - 1] & 3) << 4];
      result += "==";
    }
    if (buf.length % 3 === 2) {
      result += b64dict[buf[buf.length - 2] >> 2];
      result += b64dict[(buf[buf.length - 2] & 3) << 4 | buf[buf.length - 1] >> 4];
      result += b64dict[(buf[buf.length - 1] & 15) << 2];
      result += "=";
    }
    return result;
  }
...
var APIClient = class {
  ...
  handleTransportClose(code, reason) {
    ...
    const errorObj = { type: "error", message: msg };
    this.onError?.(errorObj);
    console.error(msg);
  }
  handleTransportError(error) {
    const errorObj = { type: "error", message: error.message };
    this.onError?.(errorObj);
...
if (typeof window !== 'undefined') {
  ...
  window.MessagePortTransport = MessagePortTransport;
}
