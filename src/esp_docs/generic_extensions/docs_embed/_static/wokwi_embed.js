(function() {
  ...
  function setupWokwiClient(iframe) {
    ...
    var messageHandler = function(event) {
      ...
      if (typeof window.MessagePortTransport === 'undefined' || typeof window.APIClient === 'undefined') {
        console.error('Wokwi client library not loaded');
        return;
      }

      try {
        var transport = new window.MessagePortTransport(event.data.port);
        var client = new window.APIClient(transport);
