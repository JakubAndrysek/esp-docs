...
class WokwiDirective(Directive):
    ...
    def run(self):
        ...
        node["allowfullscreen"] = (
            cfg.wokwi_default_allowfullscreen
...
class WokwiTabsDirective(Directive):
    ...
    def _collect_panels(
        ...
        def make_panel(
            label: str, content_nodes: List[nodes.Node], active: bool
        ) -> TabPanelNode:
            panel = TabPanelNode()
            panel["label"] = label
            panel["active"] = active
        ...
        while i < len(children):
            ...
            if isinstance(cur, WokwiNode):
                label = (
                    cur.get("tab_label") or cur.get("title") or f"Wokwi {len(panels)+1}"
                )
        ...
        json_prefix_local = json_prefix or getattr(
            self.state.document.settings.env.app.config, "wokwi_json_prefix", None
        )
        ...
        for idx, panel in enumerate(panels, start=1):
            if len(panel.children) == 1 and isinstance(panel.children[0], WokwiNode):
                ...
                if not wn.get("diagram") and json_prefix_local:
                    slug = slug_lower(
                        panel.get("label") or wn.get("tab_label") or f"wokwi-{idx}"
                    )
                    wn["diagram"] = f"{json_prefix_local}/diagram-{slug}.json"
                # Inside tabs -> no inner header
    ...
    def run(self):
        ...
        if not panels:
            errors.append(
                self.state_machine.reporter.error(
                    "wokwi-tabs: no child blocks found. Add '.. wokwi::', '.. code-block::', or '.. literalinclude::' with :name:",
                    line=self.lineno,
                )
            )
        if errors:
        ...
        # Propagate tabs-level overrides to Wokwi children
        for p in panels:
            for ch in p.children:
                if isinstance(ch, WokwiNode):
                    ...
                    if "class" in self.options:
                        ch["classes"] = list(
                            dict.fromkeys(ch.get("classes", []) + self.options["class"])
                        )

        env = getattr(self.state.document.settings, "env", None)
        serial = (
            env.new_serialno("wokwi-tabs")
            if env and hasattr(env, "new_serialno")
            else id(self)
        )
